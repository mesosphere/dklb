...
By design, pools exposing Kubernetes services to _outside_ the DC/OS cluster (i.e. pools using the `slave_public` role) must be scheduled onto the DC/OS host network (i.e. the network where the public DC/OS agents are running on top of).

Also by design, pools exposing Kubernetes services to _inside_ the DC/OS cluster must be scheduled onto a https://docs.mesosphere.com/1.12/networking/SDN/[DC/OS virtual network].
